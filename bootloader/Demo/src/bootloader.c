...
{
	csi_pin_set_mux(PB04, PB04_UART2_TX);		//TX  
...
{
	csi_pin_set_mux(PB07, PB07_UART1_TX);		//TX  48	
	csi_pin_set_mux(PB06, PB06_UART1_RX);		//RX   1
...
uint8_t bootloader_wait_syncpin_release(void)
{
	...
	csi_pin_set_mux(PB06, PB06_INPUT);	
	while(1)
	{				
		boot_mdelay(2);
		if(csi_pin_read(PB06) == PIN_CHECK_VALUE) {
			wHighTime++;
			wLowTime = 0;
		}
		else {
